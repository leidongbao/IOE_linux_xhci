 ...
 */
static int event_ring_seg_num = 1;
module_param(event_ring_seg_num, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(event_ring_seg_num, "the event ring segment number");
...
/* xhci_verify_mark
  * for multiple event ring segment, it will be best to do this verification in this host
  * controller driver. so we introduce an module param to define the event ring
  * segment number, and this module param will replace the ERST_NUM_SEGS macro
  * in xhci.h
  */
int xhci_mem_init(struct xhci_hcd *xhci, gfp_t flags)
{
	...
	xhci->event_ring = xhci_ring_alloc(xhci, event_ring_seg_num, 1, TYPE_EVENT,
						flags);
	if (!xhci->event_ring)
		goto fail;
	...
	xhci->erst.entries = dma_alloc_coherent(dev,
			sizeof(struct xhci_erst_entry) * event_ring_seg_num, &dma,
			GFP_KERNEL);
	if (!xhci->erst.entries)
		goto fail;
	xhci_dbg_trace(xhci, trace_xhci_dbg_init,
	...
	memset(xhci->erst.entries, 0, sizeof(struct xhci_erst_entry)*event_ring_seg_num);
	xhci->erst.num_entries = event_ring_seg_num;
	...
	for (val = 0, seg = xhci->event_ring->first_seg; val < event_ring_seg_num; val++) {
